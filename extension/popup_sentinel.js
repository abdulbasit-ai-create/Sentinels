...
function renderResult(result) {
  ...
  // ── Safety Report ──
  const reportSection = document.getElementById('safetyReportSection');
...
// ── ELI5 Card (Explain Like I'm 5) ──
function renderEli5Card(result) {
  const card = document.getElementById('eli5Card');
  const textEl = document.getElementById('eli5Body');
  const emojiEl = document.getElementById('eli5Emoji');
  if (!card || !textEl) return;

  const eli5 = result.eli5 || generateEli5Fallback(result.score, result.verdict, result.flags || []);
  textEl.textContent = eli5;

  const emojis = { 'SAFE': '🛡️', 'SUSPICIOUS': '⚠️', 'SCAM': '🚫' };
  if (emojiEl) emojiEl.textContent = emojis[result.verdict] || '🧠';

  card.className = 'eli5-card ' + (result.verdict ? result.verdict.toLowerCase() : '');
...
function renderQuickAnswers(result) {
  ...
  const questions = [
    { id: 'verdict', label: 'Why this verdict?', prompt: 'Give a simple reason for the ' + result.verdict + ' verdict on this site.' },
    { id: 'data', label: 'What data is collected?', prompt: 'What kind of data might this site collect about visitors?' },
    { id: 'payments', label: 'Payment safety?', prompt: 'Is it safe to enter payment info on this site?' },
    { id: 'trust', label: 'Is it trustworthy?', prompt: 'Explain trust signals this site shows or lacks.' },
    { id: 'compare', label: 'Compare to known sites', prompt: 'What type of legitimate site does this most resemble, if any?' }
  ];
...
async function askQuickQuestion(btn, key, result) {
  ...
  try {
    ...
    if (response?.success && response.answer) {
      ...
      const answers = {
        'verdict': 'Based on our analysis, this site scored ' + result.score + '/100 with verdict: ' + result.verdict + '. ' + (result.summary || ''),
        'data': result.flags && result.flags.some(f => /password|form|input/i.test(f))
          ? 'This page contains input forms that could collect sensitive data. Be cautious.'
          : 'No obvious data collection forms detected.',
        'payments': result.score >= 70
          ? 'The site appears safe for payments, but always verify the URL and SSL cert.'
          : 'We advise against entering payment information on this site.',
        'trust': result.score >= 70
          ? 'This site shows positive trust signals.'
          : 'This site lacks key trust signals. Exercise caution.',
        'compare': result.score >= 70
    ...
    loadingEl.style.display = 'none';
    answerEl.textContent = 'Could not generate answer. Check your connection.';
    answerEl.style.display = 'block';
  }
...
// ── Safety Checklist ──
function renderSafetyChecklist(result) {
  const section = document.getElementById('safetyChecklistSection');
  ...
  const { score, verdict, hasSSL, domainAge, isPhishing, flags } = result;
  const checklistKey = 'sent_checklist_' + (result.url || '');
  const saved = JSON.parse(localStorage.getItem(checklistKey) || '{}');

  const items = [
    { id: 'ssl', label: 'Site uses HTTPS/SSL', checked: !!hasSSL, auto: true },
    { id: 'domain_age', label: domainAge != null && domainAge >= 30 ? 'Domain is established (30+ days)' : 'Domain is established (30+ days)', checked: domainAge != null && domainAge >= 30, auto: true },
    { id: 'not_phishing', label: 'Not flagged in phishing databases', checked: !isPhishing, auto: true },
    { id: 'score_ok', label: 'Safety score is acceptable (60+)', checked: score >= 60, auto: true },
    { id: 'no_red_flags', label: 'No critical red flags detected', checked: !(flags && flags.length > 2), auto: true },
    { id: 'review_terms', label: 'Reviewed terms and privacy policy', checked: saved['review_terms'] || false, auto: false },
    { id: 'verified_elsewhere', label: 'Verified site through external sources', checked: saved['verified_elsewhere'] || false, auto: false }
  ];
  ...
  items.forEach((item) => {
    ...
    div.dataset.checklistId = item.id;
  ...
  function updateChecklistProgress() {
    ...
    container.querySelectorAll('.checklist-item').forEach(el => {
      if (el.classList.contains('checked')) count++;
    });
    const pct = Math.round((count / items.length) * 100);
    if (progressFill) progressFill.style.width = pct + '%';
    if (progressText) progressText.textContent = count + '/' + items.length;
  }
...
function generateSafetyReport() {
  ...
  const lines = [
    ...
    '',
    '\u2500\u2500 Security \u2500\u2500',
    'SSL:      ' + (r.hasSSL ? '\u2713 Secure' : '\u2717 No SSL'),
    'Phishing: ' + (r.isPhishing ? '\u26a0 FLAGGED' : '\u2713 Clean'),
    'Domain:   ' + (r.domainAge != null ? r.domainAge + ' days old' : 'Unknown'),
    '',
    '\u2500\u2500 Flags \u2500\u2500'
  ];
  ...
  if (flags.length === 0) {
    ...
    flags.slice(0, 6).forEach(f => lines.push('  \u2022 ' + f));
  ...
  lines.push('');
  lines.push('Generated by Sentinels v1.0');

  output.textContent = lines.join('\n');
  output.style.display = 'block';

  let copyBtn = output.querySelector('.copy-btn');
  if (!copyBtn) {
    ...
    copyBtn.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(output.textContent.replace(/\s*\ud83d\udccb Copy Report$/, ''));
...
function hideAll() {
  ...
  const eli5Card = document.getElementById('eli5Card');
  if (eli5Card) eli5Card.style.display = 'none';
  const qaSection = document.getElementById('quickAnswersSection');
  if (qaSection) qaSection.style.display = 'none';
  const checklistSection = document.getElementById('safetyChecklistSection');
